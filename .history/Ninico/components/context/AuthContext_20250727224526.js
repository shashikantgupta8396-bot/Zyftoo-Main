...
export const AuthProvider = ({ children }) => {
  ...
  // Login function - Updated to handle both API login and direct context update
  const login = async (credentialsOrUserData, tokenParam = null) => {
    try {
      console.log('🔄 [AuthContext] Starting login process...');
      console.log('📝 [AuthContext] Parameters received:', {
        isUserDataProvided: !!credentialsOrUserData,
        isTokenProvided: !!tokenParam,
        hasUserType: !!credentialsOrUserData?.userType
      });

      // Check if this is a direct context update (userData + token provided)
      if (credentialsOrUserData && tokenParam && credentialsOrUserData.id) {
        console.log('✅ [AuthContext] Direct context update mode');
        
        const userData = credentialsOrUserData;
        ...
        dispatch({
          type: AUTH_ACTIONS.LOGIN_SUCCESS,
          payload: { user: userData, token, permissions }
        });

        console.log('✅ [AuthContext] Context updated successfully for:', userData.userType);
        return { success: true, user: userData };
      }

      // Traditional API login (for backward compatibility)
      console.log('🔄 [AuthContext] API login mode');
      dispatch({ type: AUTH_ACTIONS.LOGIN_START });

      const response = await apiService.post('/auth/login', credentialsOrUserData);
