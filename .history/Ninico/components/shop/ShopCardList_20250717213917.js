            ...
            <div className="row mb-40">
                ...
                <div className="col-lg-8 col-md-12">
                    <div className="filter-product ml-20 pt-30">
                        <h3 className="filter-product-title"><Link href={`/shop-details/${item.id}`}>{item.title}</Link></h3>
                        <div className="tpproduct__ammount">
                            <span>₹{item.price?.max ? item.price.max * 100 : 31.00}</span>
                            <del>₹{item.oldPrice?.max ? item.oldPrice.max * 100 : 25.00}</del>
                        </div>
