    ...
    async function fetchData() {
      ...
      try {
        ...
        const [productsResponse, categoriesResponse] = await Promise.all([
          get(PRODUCT.GET_ALL),
          get(CATEGORY?.GET_ALL || '/api/categories') // Fallback if CATEGORY endpoint not defined
        ])
      ...
      filtered = filtered.filter(product => {
        ...
        return product.categories.some(cat => {
          if (typeof cat === 'object' && cat.name) {
            ...
            const categoryDisplay = `Category ${cat.substring(0, 8)}...`
            return categoryDisplay === filters.category
          }
          return false
        })
      })
      ...
      if (product.categories && Array.isArray(product.categories)) {
        product.categories.forEach(cat => {
          if (cat && typeof cat === 'object' && cat.name) {
            // Build hierarchical name for populated categories
            const hierarchicalName = getHierarchicalCategoryName(cat)
            categories.add(hierarchicalName)
          } else if (typeof cat === 'string') {
            // Handle ObjectId strings - you might want to fetch category names
            categories.add(`Category ${cat.substring(0, 8)}...`)
          }
        })
      }
    })
