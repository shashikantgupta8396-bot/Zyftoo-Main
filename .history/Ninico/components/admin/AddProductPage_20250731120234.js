...
export default function AddProductPage({ onNavigate }) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      setAvailableCategories(sortedCategories) // Use setAvailableCategories instead of setCategories
      setError('')
    } catch (err) {
  ...
  // Update the useEffect to only call fetchCategories
  useEffect(() => {
    fetchCategories()
  }, [])
  ...
  const renderCategoriesTab = () => (
    <div className="row">
      ...
      ) : availableCategories.length === 0 ? ( // Changed from categories.length to availableCategories.length
        <div className="col-12">
                ...
                <div className="row">
                  ...
                  <div className="col-md-6">
                    <div className="mb-3">
                      ...
                      <select
                        className="form-select"
                        value={selectedSubcategories[0] || ''} // Show first selected subcategory
                        onChange={(e) => {
  ...
  // Update the helper functions to use availableCategories
  const getMainCategories = () => {
    return availableCategories.filter(cat => !cat.parent);
  };

  const getSubcategoriesForCategory = (parentId) => {
    return availableCategories.filter(cat => cat.parent && cat.parent._id === parentId);
  };
