...
export default function CorporateSignIn() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            // First get phone number from email
            const userCheckResponse = await get(`${AUTH.CHECK_USER_EMAIL}/${email}`)
            if (!userCheckResponse.data.exists) {
                setToast('No corporate account found with this email')
                return
            }

            const phone = userCheckResponse.data.phone
            ...
            if (responseData.user.userType !== 'Corporate') {
    ...
    const handleSignUp = async (e) => {
        ...
        try {
            ...
            const encryptedOtpData = encryptData(otpData)
            const otpResponse = await post(OTP.SEND, { encryptedData: encryptedOtpData }, false)
    ...
    const handleOtpVerification = async () => {
        ...
        try {
            ...
            const encryptedRegData = encryptData(registrationData)
            const signupResponse = await post(AUTH.SIGNUP, { encryptedData: encryptedRegData }, false)
    ...
    const handleForgotPassword = async () => {
        ...
        try {
            ...
            const otpData = { phone, purpose: 'reset' }
            const encryptedData = encryptData(otpData)
    ...
    const handlePasswordReset = async () => {
        ...
        try {
            ...
            const encryptedOtpData = encryptData(otpData)
            const otpResponse = await post(OTP.VERIFY, { encryptedData: encryptedOtpData }, false)
            ...
            const encryptedResetData = encryptData(resetData)
            const resetResponse = await post(AUTH.RESET_PASSWORD, { encryptedData: encryptedResetData }, false)
