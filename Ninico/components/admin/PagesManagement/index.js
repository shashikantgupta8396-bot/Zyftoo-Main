...
export default function PagesManagement({ onNavigate }) {
  ...
  const ADMIN_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY4NzZhYTJmYzc3YWY4ODU3Njk1ZDA5MiIsInJvbGUiOiJhZG1pbiIsImlhdCI6MTc1Mzk0ODIyOSwiZXhwIjoxNzU0NTUzMDI5fQ.grgMFQdpZahvQ7vCCJeFGhtk7Jj4ttHqkovfAWWavQk'
  ...
  const loadPageConfig = async (pageKey) => {
    try {
      setLoading(true)
      const response = await fetchWithAdminToken(`/pages/${pageKey}`);
      const data = await response.json();
      console.log('Page config response:', data)
  ...
  const loadCategories = async () => {
    try {
      ...
      const response = await fetchWithAdminToken(CATEGORY.GET_ALL);
      const data = await response.json();
      if (!data.success) {
  ...
  const loadSubcategories = async () => {
    try {
      ...
      const response = await fetchWithAdminToken(SUBCATEGORY.GET_ALL);
      const data = await response.json();
      if (!data.success) {
