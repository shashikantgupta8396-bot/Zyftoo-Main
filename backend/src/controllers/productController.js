...
const createProduct = asyncHandler(async (req, res) => {
  try {
    ...
    const product = new Product({
      ...
      // Analytics (optional - will use defaults if not provided)
      analytics: analytics || {
        views: {
          total: 0,
          individual: 0,
      ...
      // Admin Management
      createdBy: req.user?.id || req.adminUser?.id,
...
const getAllProducts = asyncHandler(async (req, res) => {
  try {
    ...
    if (isAdminRequest) {
      ...
      // Flag filters
      if (is_featured === 'true' || is_featured === true || is_featured === 1) filter.is_featured = true;
      if (is_popular === 'true' || is_popular === true || is_popular === 1) filter.is_popular = true;
      if (is_trending === 'true' || is_trending === true || is_trending === 1) filter.is_trending = true;
